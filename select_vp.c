...
/**
 * Function that takes a sample from the X space and finds the best vantage point
 */
int select_vp(double* X, int n, int dimensions, int sample_size){

    if(n<sample_size){
        return 0;
    }

    //Arrays that store the indices of the random samples P and D
    int* p = malloc(sample_size*sizeof(int));
    int* d = malloc(sample_size*sizeof(int));

    srand(time(NULL));

    //Selecting the two random samples
    for(int i=0; i<sample_size; i++){
        p[i] = rand()%n;
        d[i] = rand()%n;
    }

    //Array with the coordinates of the points in the sample D
    double* d_sample = malloc(sample_size*dimensions*sizeof(double));

    for(int i=0; i<sample_size; i++){
        for(int j=0; j<dimensions; j++){
            d_sample[i*dimensions + j] = X[d[i]*dimensions +j];
        }
    }
    ...
    //Stores the coordinates of a point in P
    double* p_temp = malloc(dimensions*sizeof(double));

    //Useless variable, used only to call quick_select
    int* indices = calloc(sample_size, sizeof(int));


    int best_p;
    double best_spread = 0;

    //Itarate for every point in the sample P
    for(int i=0; i<sample_size; i++){

        //Taking a point from the sample
        for(int j=0; j<dimensions; j++){
            p_temp[j] = X[p[i]*dimensions + j];
        }

        //Finding the distances with the D sample
        double* dist = create_D(d_sample, p_temp, 1, sample_size, dimensions);

        //Selecting the median
        double mu = quick_select(dist, indices, sample_size, sample_size/2+1);

        if (mu < 0){
            printf("Hey boy something wrong here\n");
        }

        double spread = 0;

        for(int j=0; j<sample_size; j++){
            spread += (sqrt(dist[j]) - sqrt(mu)) * (sqrt(dist[j]) - sqrt(mu));
        }
    ...
    //Last check before returning best_p
    if(best_p > n-1){
        best_p = 0;
    }
...
void tree_create(double* X_sub, int* indices_global, int x_rows, int d, int root_key, double* bucket_pointer, int level_count, int final_levels){
    ...
    if(level_count<final_levels){
        ...
        //Selecting my vantage point and returning its index
        int selected = select_vp(X_sub, x_rows, d, 5);


        //Creating the root point of this sub tree into the array
        double* root_pointer = vp_elements_pointer + (3+d)*root_key;

        root_pointer[0] = indices_global[selected];             //As we said, the first number is the index of this point
        root_pointer[1] = root_key;                             //The second one is the key
    

        double* vp_coordinates = malloc(d*sizeof(double));

        for(int i=0; i<d; i++){
            root_pointer[3+i] = X_sub[selected*d + i];           //Copying the coordinates of the point into the tree
            vp_coordinates[i] = X_sub[selected*d + i];
        }
        //Finished creating for the moment. I will later have to put the median distance in root_pointer[2] as well.
  

        //Subtracting the vantage point from the X_sub
        double* new_X = malloc((x_rows-1)*d*sizeof(double));
        int* new_indices_global = malloc((x_rows-1)*d*sizeof(int));

        //When flag becomes 1 that means I passed the selected vantage point
        int flag = 0;
        ...
        for(int i=0; i<x_rows; i++){
            ...
            if(flag){
                new_indices_global[(i-1)] = indices_global[i];
            }
            else{
                new_indices_global[i] = indices_global[i];
            }

            for(int j=0; j<d; j++){

                //Checking if I passed the selected point
                if(flag){
                    new_X[(i-1)*d +j] = X_sub[i*d +j];
                    
                }
                else{
                    new_X[i*d +j] = X_sub[i*d +j];
        ...
        double* D = create_D(new_X, vp_coordinates, 1, x_rows-1, d);
        ...
        for(int i=0; i<left_rows; i++){
            ...
            indices_left[i] = new_indices_global[i];
            index_change = indices_local[i] *d;

            for(int j=0; j<d; j++){
                X_left[j +i*d] = new_X[index_change+ j];
            }
        }

        for(int i=0; i<right_rows; i++){
            indices_right[i] = new_indices_global[i+ left_rows];
		    index_change = indices_local[i + left_rows]*d;

		    for(int j=0; j<d; j++){
			    X_right[i*d +j] = new_X[index_change+ j];
		    }
	    }
